...
import { Input } from "./app/components/PasswordInput";
...
export default function App() {
  ...
  switch (press) {
    ...
    case "/auth/parameters":
      ...
      case "/auth/snap/send":
        return (
          <View style={[styles.container, { display: "flex", flexDirection: "column" }]}>
            {friends.map((friend) => (
              <Friends friend={friend.username} onClick={() => send(friend._id)} key={friend._id} />
            ))}
            <TextInput />
            <Button
              label="<"
